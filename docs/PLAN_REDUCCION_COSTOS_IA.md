...
5. **Loop acotado.** `AI_MAX_TOOL_ITERATIONS=3`, sin tool calls paralelas, con candado de `tool+argumentos` repetidos y un rescate final sin herramientas. GPT-5.5 Chat Completions usa reasoning `none` cuando hay function tools (requisito de la API), `low` sin tools y `medium` en el rescate.
