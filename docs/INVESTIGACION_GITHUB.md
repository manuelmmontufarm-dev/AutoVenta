...
> Frentes: (1) stacks WhatsApp Cloud API, (2) bots de ventas/commerce completos, (3) dominio llantas, (4) PDF/Sheets/agente GPT/dashboard.
...
(Cloud API directo + agente GPT con tools) coincide con lo que hace la punta
del ecosistema.
...
| Loop del agente (tools) | SDK oficial de OpenAI con function calling | MIT ✅ | Loop propio corto, sin framework. Memoria de conversación: array `messages` por teléfono en Postgres (JSONB), propia y trivial. |
...
- [skorokithakis/stavrobot](https://github.com/skorokithakis/stavrobot) (185★, **AGPL** ⚠️) — stack exacto (WhatsApp+LLM+Postgres), memoria en 3 niveles. Referencia arquitectónica; no copiar código.
...
1. **SDK + loop propio** (plan actual de PLAN_DESARROLLO.md): `whatsapp-api-js` + function calling de OpenAI + Express. Menos dependencias, control total, todo el bot ~1.500 líneas nuestras.
2. **BuilderBot como esqueleto**: flujos/estado/cola por conversación ya resueltos + provider Meta oficial; se le inyecta el loop de GPT. Más rápido para el demo, comunidad hispana, y permite demo con Baileys → producción con Meta sin tocar lógica.
