...
describe.sequential("Seguimientos perezosos y redacción con IA", () => {
   ...
   *
   * Durante una semana esto se arregló cancelando el envío. El 26-ago Manuel y
   * Joaquín lo cambiaron: el seguimiento SÍ sale —el cliente que no volvió a
   * escribir agradece el recordatorio—, pero confirmando. Lo que se prueba aquí
   * es que sale marcado como confirmación; el texto exacto lo fija
   * `followUpMessages` y se prueba en su propia batería.
   */
  it("con día y local ya confirmados, el seguimiento sale CONFIRMANDO, no preguntando", async () => {
    ...
    expect(enviados).toHaveLength(1);
    const [job] = await appSql<{ status: string; payload: Record<string, unknown> }[]>`
      select status, payload from follow_up_jobs
      where conversation_id = ${conv.id} and type = 'in_window_first'
    `;
    expect(job.status).toBe("sent");
    expect(job.payload.modo).toBe("confirmacion_de_visita");
  });
  ...
  it("«al sur por favor el viernes» queda registrado y el seguimiento ya no repregunta", async () => {
    ...
    // Sale, pero en modo confirmación: los hechos quedaron registrados y el
    // seguimiento ya no tiene nada que preguntar.
    expect(enviados).toHaveLength(1);
    const [job] = await appSql<{ status: string; payload: Record<string, unknown> }[]>`
      select status, payload from follow_up_jobs
      where conversation_id = ${conv.id} and type = 'in_window_first'
