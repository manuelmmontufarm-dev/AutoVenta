...
import type { LoadSpeed } from "../domain/loadSpeed.js";
...
function warrantyFor(brand: string): { golpesMeses: number; fabricaAnios: number } {
  return business.warranties[brand] ?? business.warranties.default;
}
...
function availabilityBadge(available: boolean, size = 20): SatoriNode {
  return el(
    {
      alignItems: "center",
      gap: 10,
      backgroundColor: available ? "#e8f4ea" : "#fdf0e0",
      borderRadius: 999,
    ...
    el({
      ...
      borderRadius: 999,
      backgroundColor: available ? GREEN : "#e08a00",
    }),
    text(
      { fontSize: size, fontWeight: 700, color: available ? "#1d7268" : "#a36400" },
        ...
        line.loadSpeed
          ? el(
              ...
              text(
                ...
                line.loadSpeed.translation
                  ? `${line.loadSpeed.label} = ${line.loadSpeed.translation}`
                  : line.loadSpeed.label,
              ),
            )
          : null,
        el({}, availabilityBadge(line.available, 22)),
      ),
    ),
...
function warrantyRow(brand: string): SatoriNode {
  const warranty = warrantyFor(brand);
  return el(
    { flexDirection: "column", gap: 16, padding: "34px 44px 0", width: "100%" },
    sectionLabel("Garantías"),
    el(
      { gap: 24, width: "100%" },
      medallion(String(warranty.golpesMeses), "meses", "Garantía contra golpes"),
      medallion(String(warranty.fabricaAnios), "años", "Garantía de fábrica"),
    ),
  );
}
      ...
      ...data.lines.map((line) =>
        el(
          ...
          el(
            ...
            el({}, availabilityBadge(line.available, 15)),
          ),
          el(
            { flexDirection: "column", alignItems: "flex-end", gap: 6 },
...
export async function renderQuoteImage(data: QuoteRenderData): Promise<Buffer> {
  ...
  const children: Child[] = [
    ...
    ...(single ? [warrantyRow(first.brand)] : []),
    el({ flexGrow: 1 }),
...
function compareColumn(line: RenderLine, colWidth: number): SatoriNode {
  const warranty = warrantyFor(line.brand);
  const discount =
    line.pvpConIva && line.pvpConIva > line.unitConIva
      ? Math.round((1 - line.unitConIva / line.pvpConIva) * 100)
      : null;
  return el(
    ...
    availabilityBadge(line.available, 17),
    el(
      { flexDirection: "column", alignItems: "center", gap: 4, paddingTop: 6 },
      text(
        { fontSize: 17, fontWeight: 700, color: NAVY },
        `${warranty.golpesMeses} meses contra golpes`,
      ),
      text({ fontSize: 17, color: MUTED }, `${warranty.fabricaAnios} años de fábrica`),
      line.loadSpeed
        ? text({ fontSize: 16, color: MUTED, textAlign: "center" }, line.loadSpeed.translation
            ? `${line.loadSpeed.label} · ${line.loadSpeed.translation}`
            : line.loadSpeed.label)
        : null,
    ),
...
export async function toRenderLine(input: {
  brand: string;
  design: string;
  sizeLabel: string;
  loadSpeed: LoadSpeed | null;
  quantity: number;
  /** Precio SIN IVA (como viene del catálogo). */
  priceSinIva: number;
  pvpSinIva: number | null;
  stock: number;
  photoUrl?: string | null;
}): Promise<RenderLine> {
  const iva = 1 + business.taxRate;
  const round2 = (n: number) => Math.round(n * 100) / 100;
  return {
    brand: input.brand,
    design: input.design,
    sizeLabel: input.sizeLabel,
    loadSpeed: input.loadSpeed,
    quantity: input.quantity,
    unitConIva: round2(input.priceSinIva * iva),
    pvpConIva: input.pvpSinIva ? round2(input.pvpSinIva * iva) : null,
    available: input.stock >= input.quantity && input.stock > 0,
    photo: await productPhoto(input.brand, input.design, input.photoUrl),
  };
}
