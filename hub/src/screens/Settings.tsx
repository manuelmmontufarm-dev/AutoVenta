...
function FollowUpSettingsPanel() {
  ...
  return <div className="grid gap-4 xl:grid-cols-2">
    <section className="glass rounded-3xl p-5"><p className="microlabel">Horarios</p><div className="grid gap-3 sm:grid-cols-3"><Field label="Timezone"><input className="settings-input" value={policy.timezone} onChange={(e) => setPolicy({ ...policy, timezone: e.target.value })} /></Field><Field label="Inicio"><input type="time" className="settings-input" value={policy.business_hours["1"]?.open ?? "08:30"} onChange={(e) => setPolicy({ ...policy, business_hours: Object.fromEntries(Object.entries(policy.business_hours).map(([day, hours]) => [day, hours ? { ...hours, open: e.target.value } : null])) })} /></Field><Field label="Fin"><input type="time" className="settings-input" value={policy.business_hours["1"]?.close ?? "17:30"} onChange={(e) => setPolicy({ ...policy, business_hours: Object.fromEntries(Object.entries(policy.business_hours).map(([day, hours]) => [day, hours ? { ...hours, close: e.target.value } : null])) })} /></Field></div><div className="mt-3 flex flex-wrap gap-2">{["Dom","Lun","Mar","Mié","Jue","Vie","Sáb"].map((label, day) => <label key={label} className="flex items-center gap-1 rounded-full border px-2 py-1 text-[10px] font-bold"><input type="checkbox" checked={Boolean(policy.business_hours[String(day)])} onChange={(e) => setPolicy({ ...policy, business_hours: { ...policy.business_hours, [day]: e.target.checked ? { open: "08:30", close: "17:30" } : null } })} />{label}</label>)}</div><div className="grid grid-cols-2 gap-3"><Field label="No enviar desde"><input type="time" className="settings-input" value={String(policy.quiet_hours.start ?? "17:30")} onChange={(e) => setPolicy({ ...policy, quiet_hours: { ...policy.quiet_hours, start: e.target.value } })} /></Field><Field label="Reanudar a las"><input type="time" className="settings-input" value={String(policy.quiet_hours.end ?? "08:30")} onChange={(e) => setPolicy({ ...policy, quiet_hours: { ...policy.quiet_hours, end: e.target.value } })} /></Field></div></section>
